package com.ihave.config.client;
...
public class MyJdbcClientDetailsService implements ClientDetailsService, ClientRegistrationService {
    ...
    private MyJdbcClientDetailsService.JsonMapper mapper = createJsonMapper();
    ...
    private static final String BASE_FIND_STATEMENT = "select client_id, " + CLIENT_FIELDS
            + " from top_oauth_client_details";

    private static final String DEFAULT_FIND_STATEMENT = BASE_FIND_STATEMENT + " order by client_id";

    private static final String DEFAULT_SELECT_STATEMENT = BASE_FIND_STATEMENT + " where client_id = ?";

    private static final String DEFAULT_INSERT_STATEMENT = "insert into top_oauth_client_details (" + CLIENT_FIELDS
            + ", client_id) values (?,?,?,?,?,?,?,?,?,?,?)";

    private static final String DEFAULT_UPDATE_STATEMENT = "update top_oauth_client_details " + "set "
            + CLIENT_FIELDS_FOR_UPDATE.replaceAll(", ", "=?, ") + "=? where client_id = ?";

    private static final String DEFAULT_UPDATE_SECRET_STATEMENT = "update top_oauth_client_details "
            + "set client_secret = ? where client_id = ?";

    private static final String DEFAULT_DELETE_STATEMENT = "delete from top_oauth_client_details where client_id = ?";
    ...
    public ClientDetails loadClientByClientId(String clientId) throws InvalidClientException {
        ...
        try {
            details = jdbcTemplate.queryForObject(selectClientDetailsSql, new MyJdbcClientDetailsService.ClientDetailsRowMapper(), clientId);
    ...
    private static MyJdbcClientDetailsService.JsonMapper createJsonMapper() {
        if (ClassUtils.isPresent("org.codehaus.jackson.map.ObjectMapper", null)) {
            ...
            return new MyJdbcClientDetailsService.Jackson2Mapper();
        }
        return new MyJdbcClientDetailsService.NotSupportedJsonMapper();
    }
